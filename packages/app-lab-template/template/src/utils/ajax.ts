...
const inBuildHandleMetaResponseInterceptors = (response) => {
    return {
        ...response,
        __meta: getMeta(`${response.config.url}${response.config.method}`),
    };
};
...
export const request = <T>(api: string, options: any = {}, prefix = ApiPrefix) => {
    ...
    const key = (`${fullApi}${options.method || 'get'}`).toLowerCase();

    if (options.meta) {
        MetaDataMap.set(key, options.meta);
        delete options.meta;
    }
